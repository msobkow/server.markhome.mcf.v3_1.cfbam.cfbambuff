// Description: Java 25 implementation of a IndexCol buffer
...
public class CFBamBuffIndexCol
	implements ICFBamIndexCol, Comparable<Object>, Serializable
{
	...
	public List<ICFBamRelationCol> getOptionalChildrenRefRelFromCol() {
		...
		ICFBamRelationCol[] targetArr = targetTable.readDerivedByFromColIdx(null, getRequiredId());
		if( targetArr != null ) {
	...
	public List<ICFBamRelationCol> getOptionalChildrenRefRelToCol() {
		...
		ICFBamRelationCol[] targetArr = targetTable.readDerivedByToColIdx(null, getRequiredId());
		if( targetArr != null ) {
			List<ICFBamRelationCol> results = new ArrayList<>(targetArr.length);
			for (int idx = 0; idx < targetArr.length; idx++) {
				results.add(targetArr[idx]);
	...
	public ICFBamIndexCol getOptionalLookupPrev() {
		...
		ICFBamIndexCol targetRec = targetTable.readDerived(null, getOptionalPrevId());
		return(targetRec);
	}
	@Override
	...
	public ICFBamIndexCol getOptionalLookupNext() {
		...
		ICFBamIndexCol targetRec = targetTable.readDerived(null, getOptionalNextId());
		return(targetRec);
	}
	@Override
	...
	public ICFBamValue getRequiredLookupColumn() {
		...
		ICFBamValue targetRec = targetTable.readDerived(null, getRequiredColumnId());
		return(targetRec);
	}
	@Override
	...
	public boolean equals( Object obj ) {
		if( obj == null ) {
			...
			if( getRequiredColumnId() != null && !getRequiredColumnId().isNull() ) {
				if( rhs.getRequiredColumnId() != null && !rhs.getRequiredColumnId().isNull() ) {
				...
				if( rhs.getRequiredColumnId() != null && !getRequiredColumnId().isNull()) {
					return( false );
				}
			}
	...
	public String toString() {
		String ret = "<CFBamBuffIndexCol" + getXmlAttrFragment() + "/>";
		return( ret );
	}
}
